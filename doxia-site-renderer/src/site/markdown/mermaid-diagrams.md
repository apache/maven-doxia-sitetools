...
In order to enable this feature one has to [add element `mermaid` to the site descriptor](../doxia-site-model/site.html#mermaid).
Make sure the site descriptor references namespace 2.1.0 or newer.

### Reference from external server
...
To just embed the hardcoded version of Mermaid into your generated site ([11.13.0 for Doxia Site Tools 2.1.0](https://github.com/apache/maven-doxia-sitetools/tree/master/doxia-site-renderer/src/main/resources/js)) just leave out the `externalJsUrl` child element.
...
One can further adjust the Mermaid configuration used for rendering with the [`config` element in the site descriptor](../doxia-site-model/site.html#mermaid). Its format is described in [MermaidConfig](https://mermaid.ai/open-source/config/setup/mermaid/interfaces/MermaidConfig.html).